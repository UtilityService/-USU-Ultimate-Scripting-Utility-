return function(utility)
	...
	local function create_esp(Character : Model)
		...
		for required, _ in next, skeleton_order do
			loaded_plrs[Character].obj["skeleton_" .. required] = esp.create_obj("Line", { Visible = false })
		end
		local plr = loaded_plrs[Character]
		plr.connection = utility.new_renderstepped(function(delta)
			...
			if settings.enabled and (Character and Character:FindFirstChild("Head")) then
				...
				if esp.onscreen then
					...
					if esp.corners then
						...
						displayname.Visible = settings.displayname and settings.realname and Character.Name ~= Character.DisplayName
	...
	local function destroy_esp(character)
		if not loaded_plrs[character] then return end
		loaded_plrs[character].connection:Disconnect()
		table.foreach(loaded_plrs[character].obj, function(i, v)
			v:Remove()
		end)
		loaded_plrs[character].chams_object:Destroy()
		loaded_plrs[character] = nil
	end
	...
	function esp_table.load()
		...
		local function loadChar(character : Model)
			if character:FindFirstChild("Head") and character:FindFirstChild("HumanoidRootPart") then
				create_esp(character)
				character.Destroying:Connect(destroy_esp)
			end
		end
		
		for _, Model in pairs(workspace:GetChildren()) do
			if Model:IsA("Model") and Model:FindFirstChild("HumanoidRootPart") and Model:FindFirstChild("Head") then
				loadChar(Model)
			end
		end
		
		workspace.ChildAdded:Connect(function(Model : Model)
			if Model:IsA("Model") and Model:FindFirstChild("HumanoidRootPart") and Model:FindFirstChild("Head") then
				loadChar(Model)
			end
